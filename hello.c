...
MODULE_AUTHOR("Buhaichuk Serhii");
MODULE_DESCRIPTION("Lab work 6");
MODULE_LICENSE("Dual BSD/GPL");

static unsigned int count_greet = 1;

module_param(count_greet, uint, S_IRUGO);

MODULE_PARM_DESC(count_greet, "Count of times 'Hello!' string will be printed");
...
{
    if (count_greet == 0 || (count_greet >= 5 && count_greet <= 10)) {
